...
from odoo.addons.ecommerce.controllers import main
...
class Ecommerce(main.Ecommerce):
    ...
    def pricelist(self, promo, **post):
        ...
        if coupon_status.get('not_found'):
            return super(Ecommerce, self).pricelist(promo, **post)
    ...
    def shop_payment(self, **post):
        ...
        return super(Ecommerce, self).shop_payment(**post)

    @http.route(['/shop/cart'], type='http', auth="public", website=True)
    def cart(self, **post):
        order = request.website.sale_get_order()
        order.recompute_coupon_lines()
        return super(Ecommerce, self).cart(**post)
    ...
    def _get_shop_payment_values(self, order, **kwargs):
        values = super(Ecommerce, self)._get_shop_payment_values(order, **kwargs)
