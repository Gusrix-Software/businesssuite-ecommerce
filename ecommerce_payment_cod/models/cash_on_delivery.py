...
class CreditPaymentAcquirer(models.Model):
    ...
    cod_config = fields.Many2one('cod.config', 'Cash on Delivery Configuraton', required=True)
...
class SaleOrdercollection(models.Model):
    ...
    def action_view_collections(self):
        action = self.env.ref('ecommerce_payment_cod.action_cod_collection_tree').read()[0]
...
class Website(models.Model):
    ...
    def get_cod_conf(self):
        cod_conf_obj = self.env.ref('ecommerce_payment_cod.payment_acquirer_cod', raise_if_not_found=False)
        payment_obj = self.env['payment.acquirer'].sudo().search([])
