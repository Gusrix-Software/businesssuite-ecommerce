...
class Website(main.Website):
    ...
    def get_switchable_related_views(self, key):
        ...
        if key == 'ecommerce.product':
            if not request.env.user.has_group('product.group_product_variant'):
                view_product_variants = request.website.viewref('ecommerce.product_variants')
    ...
    def toggle_switchable_view(self, view_key):
        ...
        if view_key in ('ecommerce.products_list_view', 'ecommerce.add_grid_or_list_option'):
            request.session.pop('ecommerce_shop_layout_mode', None)
...
class Ecommerce(http.Controller):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        filter_by_price_enabled = request.website.is_view_active('ecommerce.filter_products_price')
    ...
    def product(self, product, category='', search='', **kwargs):
        return request.render("ecommerce.product", self._prepare_product_values(product, category, search, **kwargs))
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        ProductCategory = request.env['ecommerce.product.public.category']
        ...
        view_track = request.website.viewref("ecommerce.product").track
    ...
    def pricelist_change(self, pl_id, **post):
        ...
        if (pl_id.selectable or pl_id == request.env.user.partner_id.property_product_pricelist) \
            ...
            if redirect_url and request.website.is_view_active('ecommerce.filter_products_price'):
                ...
                if min_price or max_price:
                    ...
                    try:
                        ...
                        args['min_price'] = min_price and str(
                            previous_price_list.currency_id._convert(min_price, pl_id.currency_id,
                    ...
                    try:
                        ...
                        args['max_price'] = max_price and str(
                            previous_price_list.currency_id._convert(max_price, pl_id.currency_id,
                                                                     request.website.company_id, fields.Date.today(),
                                                                     round=False)
                        )
                    except (ValueError, TypeError):
            ...
            request.session['ecommerce_current_pl'] = pl_id.id
    ...
    def cart(self, access_token=None, revive='', **post):
        ...
        if post.get('type') == 'popover':
            ...
            return request.render("ecommerce.cart_popover", values, headers={'Cache-Control': 'no-cache'})

        return request.render("ecommerce.cart", values)
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True, **kw):
        ...
        value['ecommerce.cart_lines'] = request.env['ir.ui.view']._render_template("ecommerce.cart_lines", {
            'ecommerce_order': order,
            'date': fields.Date.today(),
            'suggested_products': order._cart_accessories()
        })
        value['ecommerce.short_cart_summary'] = request.env['ir.ui.view']._render_template(
            "ecommerce.short_cart_summary", {
                'ecommerce_order': order,
            })
        return value
    ...
    def save_shop_layout_mode(self, layout_mode):
        ...
        request.session['ecommerce_shop_layout_mode'] = layout_mode
    ...
    def checkout_redirection(self, order):
        ...
        tx = request.env.context.get('ecommerce_transaction')
        ...
        for k, v in values.items():
            ...
            if k in authorized_fields and v is not None:
                ...
                if k not in ('field_required', 'partner_id', 'callback', 'submitted'):  # classic case
                    _logger.debug("ecommerce postprocess: %s value has been dropped (empty or not writable)" % k)
    ...
    def address(self, **kw):
        ...
        return request.render("ecommerce.address", render_values)
    ...
    def _get_country_related_render_values(self, kw, render_values):
        ...
        order = render_values['ecommerce_order']
        ...
        res = {
            'country': country,
            'country_states': country.get_ecommerce_states(mode=mode[1]),
            'countries': country.get_ecommerce_countries(mode=mode[1]),
        }
        return res
    ...
    def checkout(self, **post):
        ...
        values.update({'ecommerce_order': order})

        # Avoid useless rendering if called in ajax
        if post.get('xhr'):
            return 'ok'
        return request.render("ecommerce.checkout", values)
    ...
    def confirm_order(self, **post):
        ...
        request.website.sale_get_order(update_pricelist=True)
        extra_step = request.website.viewref('ecommerce.extra_info_option')
    ...
    def extra_info(self, **post):
        ...
        return request.render("ecommerce.extra_info", values)
    ...
    def shop_payment(self, **post):
        ...
        return request.render("ecommerce.payment", render_values)
    ...
    def shop_payment_get_status(self, sale_order_id, **post):
        ...
        return {
            ...
            'message': request.env['ir.ui.view']._render_template("ecommerce.payment_confirmation_status", {
                'order': order
            })
        }
    ...
    def shop_payment_confirmation(self, **post):
        ...
        if sale_order_id:
            order = request.env['sale.order'].sudo().browse(sale_order_id)
            return request.render("ecommerce.confirmation", {
                'order': order,
    ...
    def country_infos(self, country, mode, **kw):
        return dict(
            ...
            states=[(st.id, st.name, st.code) for st in country.get_ecommerce_states(mode=mode)],
...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def shop_payment_transaction(self, order_id, access_token, **kwargs):
        ...
        last_tx_id = request.session.get('__ecommerce_last_tx_id')
        last_tx = request.env['payment.transaction'].browse(last_tx_id).sudo().exists()
        if last_tx:
            PaymentPostProcessing.remove_transactions(last_tx)
        request.session['__ecommerce_last_tx_id'] = tx_sudo.id
