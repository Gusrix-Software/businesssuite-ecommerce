...
class ResConfigSettings(models.TransientModel):
    ...
    module_ecommerce_delivery = fields.Boolean("eCommerce Shipping Costs")
    ...
    group_delivery_invoice_address = fields.Boolean(string="Shipping Address",
                                                    implied_group='sale.group_delivery_invoice_address',
                                                    group='base.group_portal,base.group_user,base.group_public')
    group_show_uom_price = fields.Boolean(default=False, string="Base Unit Price",
                                          implied_group="ecommerce.group_show_uom_price",
                                          group='base.group_portal,base.group_user,base.group_public')

    module_ecommerce_digital = fields.Boolean("Digital Content")
    module_ecommerce_wishlist = fields.Boolean("Wishlists")
    module_ecommerce_comparison = fields.Boolean("Product Comparison Tool")
    module_ecommerce_gift_card = fields.Boolean("Gift Card")
    ...
    cart_abandoned_delay = fields.Float("Abandoned Delay",
                                        help="Number of hours after which the cart is considered abandoned.",
                                        related='website_id.cart_abandoned_delay', readonly=False)
    cart_add_on_page = fields.Boolean("Stay on page after adding to cart", related='website_id.cart_add_on_page',
                                      readonly=False)
    ...
    def get_values(self):
        ...
        if self.env['ir.module.module'].search([('name', '=', 'delivery')], limit=1).state in (
                'installed', 'to install', 'to upgrade'):
            sale_delivery_settings = 'internal'
            if self.env['ir.module.module'].search([('name', '=', 'ecommerce_delivery')], limit=1).state in (
                    'installed', 'to install', 'to upgrade'):
    ...
    def _onchange_sale_delivery_settings(self):
        if self.sale_delivery_settings == 'none':
            self.update({
                ...
                'module_ecommerce_delivery': False,
            })
            ...
            self.update({
                'module_delivery': True,
                'module_ecommerce_delivery': True,
            })
