...
class ProductProduct(models.Model):
    ...
    product_variant_image_ids = fields.One2many('ecommerce.product.image', 'product_variant_id',
                                                string="Extra Variant Images")

    website_url = fields.Char('Website URL', compute='_compute_product_website_url',
                              help='The full URL to access the document through the website.')

    base_unit_count = fields.Float('Base Unit Count', required=True, default=1,
    ...
    def _compute_base_unit_price(self):
        for product in self:
            if not product.id:
                ...
                product.base_unit_price = product.base_unit_count and (
                        product.price or product.lst_price) / product.base_unit_count
    ...
    def _check_base_unit_count(self):
        if any(product.base_unit_count < 0 for product in self):
            raise ValidationError(
                _('The value of Base Unit Count must be greater than 0. Use 0 to hide the price per unit on this product.'))
    ...
    def _is_sold_out(self):
        combination_info = self.with_context(
            ecommerce_stock_get_quantity=True).product_tmpl_id._get_combination_info(product_id=self.id)
