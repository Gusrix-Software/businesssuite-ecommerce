...
class Website(models.Model):
    ...
    def _default_recovery_mail_template(self):
        try:
            return self.env.ref('ecommerce.mail_template_sale_cart_recovery').id
        except ValueError:
            return False
    ...
    shop_extra_field_ids = fields.One2many('ecommerce.extra.field', 'website_id', string='E-Commerce Extra Fields')
    ...
    @tools.ormcache('self.env.uid', 'country_code', 'show_visible', 'website_pl', 'current_pl', 'all_pl', 'partner_pl',
                    'order_pl')
        ...
        if not is_public:
            ...
            if country_code:
                ...
                partner_pl = partner_pl.filtered(
                    lambda pl: pl.country_group_ids and country_code in pl.country_group_ids.mapped(
                        'country_ids.code') or not pl.country_group_ids
                )
    ...
    def _get_pricelist_available(self, req, show_visible=False):
        ...
        pricelists = website._get_pl_partner_order(isocountry, show_visible,
                                                   ...
                                                   req and req.session.get('ecommerce_current_pl') or None,
    ...
    def get_current_pricelist(self):
        ...
        pl = None
        partner = self.env.user.partner_id
        if request and request.session.get('ecommerce_current_pl'):
            # `ecommerce_current_pl` is set only if the user specifically chose it:
            #  - Either, he chose it from the pricelist selection
            #  - Either, he entered a coupon code
            pl = self.env['product.pricelist'].browse(request.session['ecommerce_current_pl'])
            if pl not in available_pricelists:
                pl = None
                request.session.pop('ecommerce_current_pl')
        if not pl:
    ...
    def _prepare_sale_order_values(self, partner, pricelist):
        ...
        salesperson_id = affiliate_id if self.env['res.users'].sudo().browse(
            affiliate_id).exists() else request.website.salesperson_id.id
        addr = partner.address_get(['delivery'])
        if not request.website.is_public_user():
            last_sale_order = self.env['sale.order'].sudo().search([('partner_id', '=', partner.id)], limit=1,
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        sale_order = self.env['sale.order'].with_company(request.website.company_id.id).sudo().browse(
        ...
        if self.env['product.pricelist'].browse(force_pricelist).exists():
            ...
            request.session['ecommerce_current_pl'] = pricelist_id
            update_pricelist = True
        else:
            pricelist_id = request.session.get('ecommerce_current_pl') or self.get_current_pricelist().id
        ...
        if not sale_order:
            ...
            sale_order = self.env['sale.order'].with_company(request.website.company_id.id).with_user(
                SUPERUSER_ID).create(so_data)
            ...
            if request.website.partner_id.id != partner.id:
                ...
                if country_code:
                    ...
                    sale_order.fiscal_position_id = request.env['account.fiscal.position'].sudo().with_company(
                        request.website.company_id.id)._get_fpos_by_region(country_id)
                else:
    ...
    def get_suggested_controllers(self):
        ...
        suggested_controllers.append((_('eCommerce'), url_for('/shop'), 'ecommerce'))
        return suggested_controllers

    def _search_get_details(self, search_type, order, options):
        result = super()._search_get_details(search_type, order, options)
        if search_type in ['products', 'product_categories_only', 'all']:
            result.append(self.env['ecommerce.product.public.category']._search_get_detail(self, order, options))
