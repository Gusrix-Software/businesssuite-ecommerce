...
class ProductRibbon(models.Model):
    _name = "ecommerce.product.ribbon"
        ...
        for record in self.filtered(lambda pl: pl.website_id and pl.company_id):
            if record.website_id.company_id != record.company_id:
                raise ValidationError(
                    _("""Only the company's websites are allowed.\nLeave the Company field empty or select a website from that company."""))


class ProductPublicCategory(models.Model):
    _name = "ecommerce.product.public.category"
    _inherit = [
    ...
    parent_id = fields.Many2one('ecommerce.product.public.category', string='Parent Category', index=True,
                                ondelete="cascade")
    parent_path = fields.Char(index=True)
    child_id = fields.One2many('ecommerce.product.public.category', 'parent_id', string='Children Categories')
    parents_and_self = fields.Many2many('ecommerce.product.public.category', compute='_compute_parents_and_self')
    ...
    def _compute_parents_and_self(self):
        for category in self:
            if category.parent_path:
                category.parents_and_self = self.env['ecommerce.product.public.category'].browse(
    ...
    def _search_get_detail(self, website, order, options):
        ...
        return {
            'model': 'ecommerce.product.public.category',
