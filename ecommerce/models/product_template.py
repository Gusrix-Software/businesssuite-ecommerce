...
class ProductTemplate(models.Model):
    ...
    website_description = fields.Html('Description for the website', sanitize_attributes=False,
    ...
    website_ribbon_id = fields.Many2one('ecommerce.product.ribbon', string='Ribbon')
    website_sequence = fields.Integer('Website Sequence', help="Determine the display order in the Website E-commerce",
                                      default=lambda self: self._default_website_sequence(), copy=False)
    public_categ_ids = fields.Many2many(
        'ecommerce.product.public.category', relation='product_public_category_product_template_rel',
    ...
    product_template_image_ids = fields.One2many('ecommerce.product.image', 'product_tmpl_id',
    ...
    base_unit_id = fields.Many2one('ecommerce.base.unit', string='Custom Unit of Measure',
    ...
    def _compute_base_unit_id(self):
        self.base_unit_id = self.env['ecommerce.base.unit']
        for template in self.filtered(lambda template: len(template.product_variant_ids) == 1):
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1, pricelist=False,
        ...
        if self.env.context.get('website_id'):
            ...
            if pricelist.discount_policy == 'without_discount':
                ...
                list_price = price
            combination_info['price_extra'] = self.env['account.tax']._fix_tax_included_price_company(
                combination_info['price_extra'], product_taxes, taxes, company_id)
            price_extra = \
                taxes.compute_all(combination_info['price_extra'], pricelist.currency_id, quantity_1, product, partner)[
                    tax_display]
    ...
    def _search_render_results(self, fetch_fields, mapping, icon, limit):
        ...
        for product, data in zip(self, results_data):
            if with_price:
                ...
                data['price'] = self.env['ir.qweb.field.monetary'].value_to_html(combination_info['price'],
                                                                                 monetary_options)
                if combination_info['has_discounted_price']:
                    data['list_price'] = self.env['ir.qweb.field.monetary'].value_to_html(
                        combination_info['list_price'], monetary_options)
            if with_image:
                data['image_url'] = '/web/image/product.template/%s/image_128' % data['id']
            if with_category and product.public_categ_ids:
                data['category'] = {
                    'extra_link_title': _('Categories:') if len(product.public_categ_ids) > 1 else _('Category:')}
