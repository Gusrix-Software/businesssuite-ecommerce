...
class ProductBrand(models.Model):
    ...
    brand_id = fields.Many2one('product.brand', 'Brand', group_expand='_group_expand')

    @api.model
    def _group_expand(self, records, domain, order):
        return self.env['product.brand'].search(domain, order=order)
